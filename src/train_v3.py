"""
Traffic demand pipeline v3 — targets 94+ leaderboard score.

Strategy:
  1. Replicate 2ps.py KFold CatBoost+LightGBM (proven ~88.9)
  2. Add explicit day-48 same-slot demand (critical for day-49 test)
  3. Hour-level calibration learned from day-49 train rows
  4. Blend full-data model + calibrated d48 baseline
"""
...
def run(output_path: Path | None = None, n_folds: int = N_FOLDS) -> dict:
    ...
    oof_cal = test["calibrated_d48"].values.copy()
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(train)):
        ...
        oof[va_idx] = va_ens
        print(f"Fold R²: {r2_score(y[va_idx], va_ens):.6f}")

    oof_r2 = r2_score(y, oof)
    print(f"\nOOF R² (full model): {oof_r2:.6f}")

    test_model = 0.7 * test_cat + 0.3 * test_lgb

    # Optimize blend: model vs hour-calibrated d48 on day-49 OOF proxy
    ...
    if d49_mask.sum() > 0:
        ...
        print(f"Day-49 proxy blend: model={best_wm:.2f}, calibrated_d48={best_wc:.2f}, R²={best_r2:.6f}")
    else:
    ...
    meta = {
        "oof_r2": oof_r2,
        "d49_proxy_r2": best_r2,
        "blend_model": best_wm,
        "blend_calibrated_d48": best_wc,
        "hour_ratios": hour_ratio.to_dict(),
        "global_ratio": global_ratio,
    }
    (OUTPUT_DIR / "submission_meta.json").write_text(json.dumps(meta, indent=2, default=str))
...
def main():
    ...
    run(output_path=args.output, n_folds=args.folds)
